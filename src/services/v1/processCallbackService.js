import {
  callbackUtil,
  clientInformationValidationUtil,
  constants,
  stringUtil,
} from '../../util/index.js';
...
const handlePaymentStatusCallback = async (
  ...
  let { paymentType, settlementDetails, clientInformation } =
    paymentDetails || {};
  let { requestType } =
    settlementDetails != null && settlementDetails.length > 0
      ? settlementDetails[0]
      : {};
  ...
  if (
    constants.ESIM_PAYMENT_TYPES_FOR_LCO_GETATTRIBUTE_API_CALL.includes(
      paymentType
    ) &&
    constants.ESIM_REQUEST_TYPES_FOR_LCO_GETATTRIBUTE_API_CALL.includes(
      requestType
    )
  ) {
    if (
      clientInformationValidationUtil.platformValidator(clientInformation) &&
      clientInformationValidationUtil.versionValidator(clientInformation)
    ) {
      helpers.attributesDetailsHelperService.getAttributesDetails({
        tokenPaymentId,
        requestType,
        paymentDetails,
        http,
