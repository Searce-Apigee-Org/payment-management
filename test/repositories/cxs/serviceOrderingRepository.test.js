...
import { serviceOrderingRepository } from '../../../src/repositories/cxs/index.js';

const lab = Lab.script();
const { describe, it, beforeEach, afterEach } = lab;
export { lab };

describe('Repository :: serviceOrderingRepository :: prepaidFiberServiceOrderAsync', () => {
  let req;
  let httpStub;
  let infoSpy;
  let debugSpy;
  ...
  it('should call http.post and log request and response', async () => {
    ...
    await serviceOrderingRepository.prepaidFiberServiceOrderAsync(req, payload);
    ...
    expect(
      infoSpy.calledWith(
        'CXS_PREPAID_FIBER_SERVICE_ORDER_ASYNC_REQUEST',
        payload
      )
    ).to.be.true();
    ...
    expect(
      debugSpy.calledWith(
        'CXS_PREPAID_FIBER_SERVICE_ORDER_ASYNC_RESPONSE',
        mockResponse
      )
    ).to.be.true();
  });
  ...
  it('should log error when http.post fails and not throw', async () => {
    ...
    expect(
      errorSpy.calledWith('CXS_PREPAID_FIBER_SERVICE_ORDER_ASYNC_FAILED', error)
    ).to.be.true();
  });
});
