...
import { communicationsRepository } from '../../../src/repositories/cxs/index.js';

const lab = Lab.script();
const { describe, it, beforeEach, afterEach } = lab;
export { lab };

describe('Repository :: communicationsRepository :: sendPaymentsEmailAsync', () => {
  let req;
  let httpStub;
  let infoSpy;
  let debugSpy;
  ...
  it('should call http.post and log request and response', async () => {
    ...
    await communicationsRepository.sendPaymentsEmailAsync(req, payload);
    ...
    expect(
      infoSpy.calledWith('CXS_PCOMMUNCATIONS_SEND_EMAIL_ASYNC_REQUEST', payload)
    ).to.be.true();

    expect(
      debugSpy.calledWith(
        'CXS_COMMUNCATIONS_SEND_EMAIL_ASYNC_RESPONSE',
        mockResponse
      )
    ).to.be.true();
  });
  ...
  it('should log error when http.post fails and not throw', async () => {
    ...
    expect(
      errorSpy.calledWith('CXS_COMMUNCATIONS_SEND_EMAIL_ASYNC_FAILED', error)
    ).to.be.true();
  });
});
