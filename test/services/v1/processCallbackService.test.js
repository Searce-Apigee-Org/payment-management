...
describe('Services :: v1 :: processCallbackService :: processCallback', () => {
  ...
  it('throws error when an internal operation fails', async () => {
    const notification = buildCallbackNotification('PaymentSessionCreated');
    const paymentDetails = buildMockPaymentEntity('BuyLoad');
    const req = buildReq({ payload: notification });

    req.mongo.customerPaymentsRepository.create = sinon
      .stub()
      .rejects(new Error('DB_ERROR'));
    req.processCallbackService = {};

    await expect(processCallback(req, paymentDetails, false)).to.reject(
      'DB_ERROR'
    );
  });
});
...
describe('Services :: v1 :: processCallbackService :: handlePaymentStatusCallback', () => {
  ...
  beforeEach(() => {
    req = {
      attributesDetailsHelperService: {
        getAttributesDetails: sinon.stub().resolves(),
      },
    };
  });
  ...
  it('triggers getAttributesDetails when both payment/request types are valid and clientInformation is validated', async () => {
    const paymentType = 'GCASH';
    const requestType = 'BuyESIM';
    const clientInformation = 'GlobeOne/1.13.0 ios|android';
    ...
    req.helpers.attributesDetailsHelperService = {
      getAttributesDetails: sinon.stub().resolves(),
    };

    await handlePaymentStatusCallback(paymentDetails, req);

    const getAttributesDetailsStub =
      req.helpers.attributesDetailsHelperService.getAttributesDetails;

    expect(getAttributesDetailsStub.calledOnce).to.be.true();
    expect(getAttributesDetailsStub.firstCall.args[0]).to.shallow.include({
      tokenPaymentId: notification.payload.notification.payload.paymentId,
      requestType: 'BuyESIM',
      paymentDetails,
    });
  });

  it('processes CSPayment when accountsForCSPayment array is populated', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.status = 'AUTHORISED';
    ...
    expect(
      req.helpers.paymentSessionCallback.processCSPayment.calledOnce
    ).to.be.true();
    expect(
      req.helpers.paymentSessionCallback.processCSPayment.calledWith(req)
    ).to.be.true();
  });

  it('processes BuyPromo when accountsForBuyPromo is populated and BuyLoad is empty', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.accounts = [
      { status: 'AUTHORISED' },
    ];
    ...
    expect(
      req.helpers.paymentSessionCallback.processPurchasePromo.calledOnce
    ).to.be.true();
  });

  it('processes BuyVoucher when preceding account arrays are empty', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.status = 'AUTHORISED';
    notification.payload.notification.payload.accounts = [
      { status: 'AUTHORISED' },
    ];
    const paymentDetails = { paymentType: 'BuyVoucher' };
    ...
    expect(
      req.helpers.paymentSessionCallback.processBuyVoucher.calledOnce
    ).to.be.true();
  });

  it('processes VolumeBoost when preceding account arrays are empty', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.accounts = [
      { status: 'AUTHORISED' },
    ];
    const paymentDetails = { paymentType: 'VolumeBoost' };
    ...
    expect(
      req.helpers.paymentSessionCallback.processVolumeBoost.calledOnce
    ).to.be.true();
  });

  it('processes PrepaidFiberServiceOrders when preceding account arrays are empty', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.accounts = [
      { status: 'AUTHORISED' },
    ];
    const paymentDetails = { paymentType: 'PrepaidFiber' };
    ...
    expect(
      req.helpers.paymentSessionCallback.processPrepaidFiberServiceOrders
        .calledOnce
    ).to.be.true();
  });

  it('processes PrepaidFiberRepairOrders when preceding account arrays are empty', async () => {
    const notification = buildCallbackNotification('PaymentProcessed');
    notification.payload.notification.payload.accounts = [
      { status: 'AUTHORISED' },
    ];
    ...
    expect(
      req.helpers.paymentSessionCallback.processPrepaidFiberRepairOrders
        .calledOnce
    ).to.be.true();
  });
});
